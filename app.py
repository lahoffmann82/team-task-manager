...
def task_row_html(tk):
    ...
    desc_html = f'<div class="task-desc-text">{desc}</div>' if desc else ""
    cmt = tk.get("comment", "")
    cmt_html = f'<div class="task-comment-text">\U0001f4ac {cmt}</div>' if cmt else ""
    att = len(get_task_attachments(tk))
    att_html = f'<div class="task-info-row"><span class="task-info-label">Anh\u00e4nge</span><span class="task-info-value">\U0001f4ce {att}</span></div>' if att else ""
    due = tk.get("due", "")
    due_html = f'<div class="task-info-row"><span class="task-info-label">F\u00e4llig</span><span class="task-info-value">{due}</span></div>' if due else ""

    return f'''<div class="task-card">
    <div class="task-left">
        <div class="task-heading">{priv}{tk["title"]}</div>
        {desc_html}{cmt_html}
    </div>
    <div class="task-right">
        <div class="task-info-row"><span class="task-info-label">Status</span><span class="badge" style="background:{sc}">{status_to_display(sk)}</span></div>
        <div class="task-info-row"><span class="task-info-label">Kategorie</span><span class="badge" style="background:{cc}">{category_to_display(ck)}</span></div>
        <div class="task-info-row"><span class="task-info-label">Priorit\u00e4t</span><span class="badge" style="background:{pc}">{priority_to_display(pk)}</span></div>
        {due_html}{att_html}
    </div>
</div>'''
    ...
    for member in team:
        ...
        with st.expander(f"**{member['name']}** \u2014 {len(mt)} {t('tasks_count')}", expanded=True):
            ...
            if amt:
                ...
                st.markdown(f'<a href="{ml}" target="_blank" style="display:inline-block;padding:5px 12px;background:#4CAF50;color:white;border-radius:4px;text-decoration:none;font-size:0.85em;">\U0001f4e7 {t("send_email_to",name=member["name"])}</a>', unsafe_allow_html=True)
            _archive_team(member, tasks, spreadsheet)
    st.divider()
    if st.button(f"\U0001f4e7 {t('send_all_emails')}"):
