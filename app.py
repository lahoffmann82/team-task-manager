...
def get_attachments_folder_id():
    """Get the shared attachments folder ID from secrets or use default."""
...
COMPACT_CSS = """
<style>
.task-row { display:flex; align-items:center; padding:6px 10px; border-bottom:1px solid #eee; gap:8px; font-size:0.9em; }
.task-row:hover { background:#f8f9fa; }
.task-title { flex:3; font-weight:600; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.task-status { flex:0.8; text-align:center; }
.task-cat { flex:0.8; text-align:center; }
.task-prio { flex:0.6; text-align:center; }
.task-due { flex:0.8; color:#666; font-size:0.85em; }
.task-att { flex:0.3; color:#666; font-size:0.85em; }
.task-actions { flex:0.8; display:flex; gap:2px; justify-content:flex-end; }
.badge { display:inline-block; padding:2px 8px; border-radius:10px; color:white; font-size:0.75em; }
.btn-sm { padding:2px 6px; border:1px solid #ddd; border-radius:4px; background:white; cursor:pointer; font-size:0.8em; }
...
def search_tasks(tasks_list, query):
    """Filter tasks by search query across title, description, comment."""
    if not query:
        return tasks_list
    q = query.lower()
    return [
        ...
        or q in task.get("comment", "").lower()
        or q in category_to_display(task.get("category", "Sonstiges")).lower()
        or q in priority_to_display(task.get("priority", "Mittel")).lower()
    ]


# ─── Attachments UI (compact) ────────────────────────────────


def render_attachments(task, tidx, spreadsheet, tasks, drive_service, prefix="att"):
    attachments = get_task_attachments(task)
    if attachments:
        links = " ".join([f"[📄 {a['name']}]({a['link']})" for a in attachments])
        st.markdown(f"📎 {links}", unsafe_allow_html=True)

    uploaded = st.file_uploader(
        t("upload_files"), accept_multiple_files=True,
        key=f"{prefix}_up_{tidx}", label_visibility="collapsed",
    )
    if uploaded:
...
def render_task_row_html(task):
    """Render a single compact task row as HTML."""
    sk = task.get("status", "Offen")
    ck = task.get("category", "Sonstiges")
    pk = task.get("priority", "Mittel")
    s_color = {"Offen": "#9e9e9e", "In Arbeit": "#2196F3", "Erledigt": "#4CAF50"}.get(sk, "#9e9e9e")
    c_color = CATEGORY_COLORS_MAP.get(ck, "#9E9E9E")
    p_color = {"Hoch": "#f44336", "Mittel": "#ff9800", "Niedrig": "#4caf50"}.get(pk, "#9e9e9e")
    is_priv = "🔒 " if task.get("private") == "ja" else ""
    due = task.get("due", "")
    att_count = len(get_task_attachments(task))
    att_str = f"📎{att_count}" if att_count > 0 else ""

    return f"""<div class="task-row">
        <div class="task-title">{is_priv}{task['title']}</div>
        <div class="task-status"><span class="badge" style="background:{s_color}">{status_to_display(sk)}</span></div>
        <div class="task-cat"><span class="badge" style="background:{c_color}">{category_to_display(ck)}</span></div>
        <div class="task-prio"><span class="badge" style="background:{p_color}">{priority_to_display(pk)}</span></div>
        <div class="task-due">{due}</div>
        <div class="task-att">{att_str}</div>
    </div>"""
...
def tab_my_tasks(spreadsheet, tasks, user, drive_service):
    ...
    search_q = sc1.text_input("🔍", placeholder=t("search_placeholder"), key="my_search", label_visibility="collapsed")
    fs = sc2.multiselect(t("status_filter"), get_status_options(), default=get_status_options(), key="my_sf")
    fc = sc3.multiselect(t("category_filter"), get_category_options(), default=get_category_options(), key="my_cf")
    fsk = [status_to_key(s) for s in fs]
    fck = [category_to_key(c) for c in fc]

    def filt(tl):
        filtered = [task for task in tl if task.get("status", "Offen") in fsk and task.get("category", "Sonstiges") in fck]
        return search_tasks(filtered, search_q)

    fb = filt(boss_tasks)
    if fb:
        st.subheader(f"📥 {t('boss_tasks', count=len(fb))}")
        # Compact HTML header
        st.markdown('<div class="task-row" style="font-weight:700;border-bottom:2px solid #333;font-size:0.8em;color:#666;"><div class="task-title">Aufgabe</div><div class="task-status">Status</div><div class="task-cat">Kat.</div><div class="task-prio">Prio</div><div class="task-due">Fällig</div><div class="task-att"></div></div>', unsafe_allow_html=True)
        for i, task in enumerate(fb):
            st.markdown(render_task_row_html(task), unsafe_allow_html=True)
            # Expandable detail
            with st.expander(f"▸ {t('edit')} / {t('status')}", expanded=False):
                c1, c2 = st.columns([1, 2])
                sk = task.get("status", "Offen")
                cur_idx = STATUS_KEYS.index(sk) if sk in STATUS_KEYS else 0
                new_s = c1.selectbox(t("status"), get_status_options(), index=cur_idx, key=f"boss_s_{i}")
                new_c = c2.text_input(t("comment"), value=task.get("comment", ""), key=f"boss_c_{i}")
                new_sk = status_to_key(new_s)
                if new_sk != sk or new_c != task.get("comment", ""):
                    if st.button(f"💾 {t('save')}", key=f"boss_sv_{i}"):
                        task["status"] = new_sk
                        task["comment"] = new_c
                        save_tasks(spreadsheet, tasks)
                        st.success(t("saved"))
                ...
                render_attachments(task, i, spreadsheet, tasks, drive_service, prefix="boss_att")
    ...
    if fp:
        ...
        for i, task in enumerate(fp):
            ...
            with st.expander(f"▸ {t('edit')} / {t('status')}", expanded=False):
                ...
                sk = task.get("status", "Offen")
                cur_idx = STATUS_KEYS.index(sk) if sk in STATUS_KEYS else 0
                new_s = c1.selectbox(t("status"), get_status_options(), index=cur_idx, key=f"priv_s_{i}")
                new_c = c2.text_input(t("comment"), value=task.get("comment", ""), key=f"priv_c_{i}")
                new_sk = status_to_key(new_s)
                if new_sk != sk or new_c != task.get("comment", ""):
                    if st.button(f"💾 {t('save')}", key=f"priv_sv_{i}"):
                        task["status"] = new_sk
                        task["comment"] = new_c
                        save_tasks(spreadsheet, tasks)
                        st.success(t("saved"))
                ...
                if task.get("description"):
                    st.caption(task["description"])
                render_attachments(task, i, spreadsheet, tasks, drive_service, prefix="priv_att")

    if not fb and not fp:
        st.info(t("no_filtered"))


# ─── Tab: Team & Emails (kompakt) ────────────────────────────
...
def tab_team_overview(spreadsheet, members, tasks, user, drive_service):
    ...
    # Search + Filters
    sc1, sc2, sc3 = st.columns([2, 1, 1])
    search_q = sc1.text_input("🔍", placeholder=t("search_placeholder"), key="team_search", label_visibility="collapsed")
    fs = sc2.multiselect(t("status_filter"), get_status_options(), default=get_status_options(), key="tsf")
    fc = sc3.multiselect(t("category_filter"), get_category_options(), default=get_category_options(), key="tcf")
    fsk = [status_to_key(s) for s in fs]
    fck = [category_to_key(c) for c in fc]
    ...
    for member in team:
        ...
        mt = search_tasks(mt, search_q)

        archived = get_archived_tasks(tasks, member["email"])
        if not mt and not archived:
            continue
        if not mt and archived:
        ...
        with st.expander(f"**{member['name']}** — {len(mt)} {t('tasks_count')}", expanded=True):
            ...
            for j, task in enumerate(mt):
                ...
                # Mini action buttons in one line
                bc1, bc2, bc3, bc4 = st.columns([1, 1, 1, 6])
                if bc1.button("⬆", key=f"up_{tidx}", help="Nach oben"):
                    if j > 0:
                        prev = mt[j-1]
                        task["sort_order"], prev["sort_order"] = prev["sort_order"], task["sort_order"]
                        save_tasks(spreadsheet, tasks)
                        st.rerun()
                if bc2.button("⬇", key=f"dn_{tidx}", help="Nach unten"):
                    if j < len(mt)-1:
                        nxt = mt[j+1]
                        ...
                        st.rerun()
                if sk == "Erledigt":
                    if bc3.button("📦", key=f"ar_{tidx}", help=t("archive")):
                        task["archived"] = "ja"
                        save_tasks(spreadsheet, tasks)
                ...
                with st.expander(f"✏️ {task['title'][:30]}...", expanded=False):
                    with st.form(f"edit_{tidx}"):
                        et = st.text_input(t("title"), value=task["title"])
                        ed = st.text_area(t("description"), value=task.get("description", ""), height=60)
                    ...
                    render_attachments(task, tidx, spreadsheet, tasks, drive_service, prefix=f"t_att")
...
def _render_archive_section(member, tasks, spreadsheet):
    ...
    label = t("hide_archive") if st.session_state[arch_key] else t("show_archive")
    icon = "📂" if st.session_state[arch_key] else "📦"
    if st.button(f"{icon} {label} ({len(archived)})", key=f"at_{member['email']}"):
    ...
    if st.session_state[arch_key]:
        for task in archived:
            ...
            st.caption(f"~~{task['title']}~~ | {category_to_display(task.get('category', 'Sonstiges'))} | 📅 {task.get('due', '-')}{att_str}")
            if att:
                st.markdown(" ".join([f"[📄 {a['name']}]({a['link']})" for a in att]))
