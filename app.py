...
Speichert Daten in Google Sheets, Anhänge in Google Drive.
...
TRANSLATIONS = {
    "de": {
        ...
        "delete": "Loeschen", "edit": "Bearbeiten", "save_changes": "Aenderungen speichern",
        ...
        "change_password": "Passwort aendern", "old_password": "Altes Passwort",
        "new_password": "Neues Passwort", "confirm_password": "Neues Passwort bestaetigen",
        "change": "Aendern", "user_not_found": "User nicht gefunden.",
        "old_pw_wrong": "Altes Passwort ist falsch.",
        "enter_new_pw": "Bitte neues Passwort eingeben.",
        "pw_mismatch": "Neue Passwoerter stimmen nicht ueberein.",
        "pw_changed": "Passwort geaendert!",
        "tab_new_task": "Neue Aufgabe", "tab_my_tasks": "Meine Aufgaben",
        "tab_team": "Aufgaben Team", "tab_import": "Import",
        "task_type": "Aufgabentyp",
        "team_task": "Team-Aufgabe (fuer Mitarbeiter)",
        "private_task": "Private Aufgabe (nur fuer mich)",
        "create_task": "Neue Aufgabe erstellen", "title": "Titel *",
        "description": "Beschreibung", "assign_to": "Zuweisen an *",
        "category": "Kategorie", "priority": "Prioritaet", "due_date": "Faellig am",
        ...
        "no_filtered": "Keine Aufgaben fuer die gewaehlten Filter.",
        ...
        "send_all_emails": "Alle Emails auf einmal oeffnen",
        "import_title": "Email-Antwort importieren",
        "import_desc": "Fuege die Antwort-Email eines Mitarbeiters hier ein.",
        "reply_from": "Antwort von Mitarbeiter",
        "paste_reply": "Email-Antwort hier einfuegen",
        "no_updates": "Keine Aufgaben-Updates erkannt.",
        "detected_changes": "Erkannte Aenderungen",
        "task_not_found": "Aufgabe [{num}] existiert nicht.",
        "no_change": "keine Aenderung",
        "unknown_status": "Unbekannter Status: '{status}'",
        "apply_changes": "{count} Aenderung(en) uebernehmen",
        "changes_applied": "Aenderungen uebernommen!",
        "no_valid_changes": "Keine gueltigen Aenderungen.",
        ...
        "delete_member_btn": "Entfernen bestaetigen",
        "delete_member_wrong_pw": "Falsches Passwort.",
        "delete_member_success": "{name} wurde entfernt.",
        "attachments": "Anhaenge", "upload_files": "Dateien hochladen",
        "no_attachments": "Keine Anhaenge",
        "file_uploaded": "Datei '{name}' hochgeladen!", "open_file": "Oeffnen",
...
def priority_to_key(d):
    o = get_priority_options()
    return PRIORITY_KEYS[o.index(d)] if d in o else PRIORITY_KEYS[0]
...
def status_to_key(d):
    o = get_status_options()
    return STATUS_KEYS[o.index(d)] if d in o else STATUS_KEYS[0]
...
def category_to_key(d):
    o = get_category_options()
    return CATEGORY_KEYS[o.index(d)] if d in o else CATEGORY_KEYS[0]
...
def render_lang_toggle():
    ...
    _, c2 = st.columns([8, 1])
    with c2:
        nl = st.selectbox("lang_globe", ["Deutsch", "English"], index=0 if lang == "de" else 1, key="lang_sel", label_visibility="collapsed")
...
COMPACT_CSS = """<style>
.task-row{display:flex;align-items:flex-start;padding:6px 8px;border-bottom:1px solid #eee;gap:6px;font-size:0.88em;min-height:32px;}
.task-row:hover{background:#f8f9fa;}
.task-title{flex:3;min-width:0;font-weight:600;overflow:hidden;}
.task-desc{color:#666;font-size:0.8em;margin-top:2px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;}
.task-comment{color:#888;font-size:0.75em;margin-top:1px;}
.task-status{width:80px;min-width:80px;text-align:center;padding-top:2px;}
.task-cat{width:80px;min-width:80px;text-align:center;padding-top:2px;}
.task-prio{width:60px;min-width:60px;text-align:center;padding-top:2px;}
.task-due{width:80px;min-width:80px;color:#666;font-size:0.85em;padding-top:4px;}
.task-att{width:30px;min-width:30px;color:#666;font-size:0.85em;padding-top:4px;}
.badge{display:inline-block;padding:2px 7px;border-radius:10px;color:white;font-size:0.75em;white-space:nowrap;}
</style>"""

TABLE_HEADER = '<div class="task-row" style="font-weight:700;border-bottom:2px solid #333;font-size:0.8em;color:#666;"><div class="task-title">Aufgabe</div><div class="task-status">Status</div><div class="task-cat">Kat.</div><div class="task-prio">Prio</div><div class="task-due">Faellig</div><div class="task-att"></div></div>'
...
def show_setup(spreadsheet):
    ...
    with st.form("setup"):
        n = st.text_input(t("your_name"))
        e = st.text_input(t("your_email"))
    ...
    with st.form("login"):
        ...
        if st.form_submit_button(t("login")):
            ...
            if not m:
                ...
                return
            if m.get("password", DEFAULT_PASSWORD) != p:
...
def render_sidebar(spreadsheet, members, tasks, user):
    ...
    with st.sidebar.form("add_m", clear_on_submit=True):
        ...
        if st.form_submit_button(t("invite")):
            if nn and ne:
                if any(m["email"].lower() == ne.lower() for m in members):
                    ...
                    st.success(f"{nn} {t('invited', pw=DEFAULT_PASSWORD)}")
                    st.rerun()
            else:
    ...
    if team:
        ...
        for i, m in enumerate(team):
            if m["email"] == ue:
                continue
            mi = members.index(m)
            st.sidebar.write(f"{'👑' if m.get('role')=='admin' else ''} **{m['name']}** ({m['email']})")
            dk = f"dc_{mi}"
            if dk not in st.session_state:
                st.session_state[dk] = False
            if not st.session_state[dk]:
                if st.sidebar.button("✕", key=f"db_{mi}"):
                ...
                with st.sidebar.form(f"df_{mi}"):
                    ...
                    if cc1.form_submit_button(f"🗑 {t('delete_member_btn')}"):
                    ...
                    if cc2.form_submit_button(t("cancel")):
                        st.session_state[dk] = False
                        st.rerun()
    ...
    with st.sidebar.form("cpw"):
        ...
        np_ = st.text_input(t("new_password"), type="password")
        ...
        if st.form_submit_button(t("change")):
            ...
            elif op != m.get("password", DEFAULT_PASSWORD):
        ...
        for ai, a in enumerate(att):
            ...
            with c1:
                ...
                if link:
                    ...
                    st.caption(f"📎 {fname}")
            with c2:
                if st.button("🗑", key=f"{prefix}_rm_{tidx}_{ai}", help=t("remove_attachment")):
                    att.pop(ai)
...
def tab_new(spreadsheet, members, tasks, user):
    ...
    tt = st.radio(t("task_type"), [f"📤 {t('team_task')}", f"🔒 {t('private_task')}"], horizontal=True, key="tt")
    isp = "🔒" in tt
    if not isp and not asgn:
        st.info(t("invite_first"))
        return

    with st.form("at", clear_on_submit=True):
        st.subheader(t("create_task"))
        ti = st.text_input(t("title"))
        de = st.text_area(t("description"), height=120)
        if isp:
            st.info(f"🔒 {user['name']}")
            ae = user["email"]
        else:
            mn = [f"{m['name']} ({m['email']})" for m in asgn]
            a = st.selectbox(t("assign_to"), mn)
        c1, c2, c3 = st.columns(3)
        ca = c1.selectbox(t("category"), get_category_options())
        pr = c2.selectbox(t("priority"), get_priority_options())
        du = c3.date_input(t("due_date"), value=None)
        if st.form_submit_button(f"✅ {t('create_btn')}", type="primary"):
            if ti:
                if not isp:
                    ae = asgn[mn.index(a)]["email"]
                ms = max((tk.get("sort_order", 0) for tk in tasks), default=0)
                tasks.append({
                    "title": ti, "description": de, "assigned_to": ae,
                    "priority": priority_to_key(pr),
                    "due": du.strftime("%d.%m.%Y") if du else "",
                    "created": date.today().strftime("%d.%m.%Y"),
                    "status": "Offen", "comment": "", "category": category_to_key(ca),
                    "sort_order": ms + 1, "created_by": user["email"],
                    "private": "ja" if isp else "", "archived": "", "attachments": "",
                })
                save_tasks(spreadsheet, tasks)
                st.success(t("private_created" if isp else "task_created", title=ti))
                st.rerun()
            else:
                st.warning(t("enter_title"))


# --- Shared task list (clickable rows) ------------------------------
...
def _task_row_label(tk):
    ...
    parts = [f"{priv}**{title}**  |  {sk}  |  {ck}  |  {pk}"]
    if due:
        parts[0] += f"  |  📅 {due}"
    if att_s:
        parts[0] += att_s
    if desc_short:
        parts.append(f"  _{desc_short}_")
    return "\n".join(parts)
...
def render_task_list(task_list, tasks, members, spreadsheet, user, drive_service, prefix="tl"):
    ...
    for j, tk in enumerate(task_list):
        ...
        rc, bc = st.columns([9, 1])
        with rc:
            if st.button(
                _task_row_label(tk),
                key=f"{prefix}_row_{tidx}",
                help=t("hint_edit"),
        ...
        with bc:
            ...
            if b1.button("↑", key=f"{prefix}_u_{tidx}", help=t("hint_move_up")):
                if j > 0:
                    p = task_list[j - 1]
                    tk["sort_order"], p["sort_order"] = p["sort_order"], tk["sort_order"]
                    save_tasks(spreadsheet, tasks)
                    st.rerun()
            if b2.button("↓", key=f"{prefix}_d_{tidx}", help=t("hint_move_down")):
                if j < len(task_list) - 1:
                    ...
                    st.rerun()
            if sk == "Erledigt":
                if b3.button("📦", key=f"{prefix}_a_{tidx}", help=t("hint_archive")):
                    tk["archived"] = "ja"
                    save_tasks(spreadsheet, tasks)
                    st.rerun()

        # Edit form (opened by clicking the task row)
        if st.session_state.get(f"eo_{prefix}_{tidx}", False):
            with st.form(f"{prefix}_ef_{tidx}"):
                ...
                # Assign-to on its own wider row
                ...
                ea = st.selectbox(t("assign_to"), an, index=an.index(ca) if ca in an else 0, key=f"{prefix}_ea_{tidx}")
                e1, e2 = st.columns(2)
                ec = e1.selectbox(t("category"), get_category_options(), index=CATEGORY_KEYS.index(tk.get("category", "Sonstiges")) if tk.get("category", "Sonstiges") in CATEGORY_KEYS else 0, key=f"{prefix}_ec_{tidx}")
                ep = e2.selectbox(t("priority"), get_priority_options(), index=PRIORITY_KEYS.index(tk.get("priority", "Mittel")) if tk.get("priority", "Mittel") in PRIORITY_KEYS else 0, key=f"{prefix}_ep_{tidx}")
                e3, e4 = st.columns(2)
                es = e3.selectbox(t("status"), get_status_options(), index=STATUS_KEYS.index(sk) if sk in STATUS_KEYS else 0, key=f"{prefix}_es_{tidx}")
                try:
                    dv = datetime.strptime(tk["due"], "%d.%m.%Y").date() if tk.get("due") else None
                except (ValueError, TypeError):
                    dv = None
                edue = e4.date_input(t("due_date"), value=dv, key=f"{prefix}_ed_{tidx}")
                ...
                if save_clicked:
                    ...
                    tk["priority"] = priority_to_key(ep)
                    tk["status"] = status_to_key(es)
...
def tab_my(spreadsheet, tasks, user, drive_service, members):
    ...
    sq = s1.text_input("🔍", placeholder=t("search_placeholder"), key="ms", label_visibility="collapsed")
    ...
    fb = fl(bt)
    if fb:
        with st.expander(f"📥 {t('boss_tasks', count=len(fb))}", expanded=True):
    ...
    fp = fl(pt)
    if fp:
        with st.expander(f"🔒 {t('private_tasks', count=len(fp))}", expanded=True):
...
def _archive_inline(archived, prefix):
    ...
    icon = "📂" if st.session_state[ak] else "📦"
    if st.button(f"{icon} {lb} ({len(archived)})", key=f"at_{prefix}"):
        st.session_state[ak] = not st.session_state[ak]
        st.rerun()
    if st.session_state[ak]:
        for tk in archived:
            att = get_task_attachments(tk)
            ats = f" 📎{len(att)}" if att else ""
            st.caption(f"~~{tk['title']}~~ | {category_to_display(tk.get('category', 'Sonstiges'))} | 📅 {tk.get('due', '-')}{ats}")
...
def tab_team(spreadsheet, members, tasks, user, drive_service):
    ...
    if not team:
        ...
        return
    if not vis and not any(get_archived(tasks, m["email"]) for m in team):
        st.info(t("no_tasks_yet"))
        return

    s1, s2, s3 = st.columns([2, 1, 1])
    sq = s1.text_input("🔍", placeholder=t("search_placeholder"), key="ts", label_visibility="collapsed")
    ...
    for member in team:
        ...
        with st.expander(f"**{member['name']}** -- {len(mt)} {t('tasks_count')}", expanded=True):
            ...
            if amt:
                ...
                st.markdown(f'<a href="{ml}" target="_blank" style="display:inline-block;padding:5px 12px;background:#4CAF50;color:white;border-radius:4px;text-decoration:none;font-size:0.85em;">📧 {t("send_email_to", name=member["name"])}</a>', unsafe_allow_html=True)
            _archive_team(member, tasks, spreadsheet)

    st.divider()
    if st.button(f"📧 {t('send_all_emails')}"):
        links = [build_mailto(m["email"], m["name"], [tk for tk in vis if tk["assigned_to"] == m["email"] and tk.get("private") != "ja"], app_url) for m in team if any(tk["assigned_to"] == m["email"] for tk in vis)]
        st.components.v1.html(f"<script>{''.join(f'window.open(\"{l}\");' for l in links)}</script>", height=0)
...
def _archive_team(member, tasks, spreadsheet):
    ...
    if ak not in st.session_state:
        st.session_state[ak] = False
    lb = t("hide_archive") if st.session_state[ak] else t("show_archive")
    if st.button(f"{'📂' if st.session_state[ak] else '📦'} {lb} ({len(ar)})", key=f"at_{member['email']}"):
        st.session_state[ak] = not st.session_state[ak]
        st.rerun()
    if st.session_state[ak]:
        for tk in ar:
...
def tab_import(spreadsheet, members, tasks, user):
    ...
    sm = st.selectbox(t("reply_from"), mn, key="im")
    ...
    for m in pattern.finditer(rt):
        ...
        for k in STATUS_KEYS:
            if k.lower() == m.group(3).strip().lower():
                sk = k
                break
        if not sk:
            for i, o in enumerate(get_status_options()):
                if o.lower() == m.group(3).strip().lower():
                    sk = STATUS_KEYS[i]
                    break
    ...
    for u in updates:
        tn = u["index"]
        if tn < 1 or tn > len(mt):
            st.warning(t("task_not_found", num=tn))
            continue
        tk = mt[tn - 1]
        os_ = tk.get("status", "Offen")
        oc = tk.get("comment", "")
        ns, nc = u["status"], u["comment"]
        iv = ns in STATUS_KEYS
        if os_ != ns or oc != nc:
            c1, c2 = st.columns(2)
            c1.write(f"**[{tn}] {tk['title']}**")
            ch = []
            if os_ != ns:
                ch.append(f"Status: {status_to_display(os_)} -> **{status_to_display(ns)}**" if iv else f"⚠️ {ns}")
            if oc != nc:
                ch.append(f"💬 _{nc}_")
            c2.write(" | ".join(ch))
            if iv:
                valid.append({"n": tn, "s": ns, "c": nc})
    if valid:
...
def main():
    ...
    with t1:
        tab_new(spreadsheet, members, tasks, user)
    with t2:
        tab_my(spreadsheet, tasks, user, drive_service, members)
    with t3:
        tab_team(spreadsheet, members, tasks, user, drive_service)
    with t4:
        tab_import(spreadsheet, members, tasks, user)
