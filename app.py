    ...
    for member in team:
        ...
        with st.expander(f"**{member['name']}** \u2014 {len(mt)} {t('tasks_count')}", expanded=True):
            ...
            if amt:
                ...
                # Email button via JavaScript mailto
                email_body = build_email_body(member["name"], amt, app_url)
                email_subj = t("email_subject", date=date.today().strftime("%d.%m.%Y"))
                # Use JS to construct mailto — avoids URL length issues in href
                import html as html_mod
                js_to = member["email"]
                js_subj = html_mod.escape(email_subj).replace("'", "\\'")
                js_body = html_mod.escape(email_body).replace("'", "\\'").replace("\n", "\\n")
                btn_id = f"mail_{member['email'].replace('@','_').replace('.','_')}"
                st.markdown(
                    f'<button id="{btn_id}" onclick="'
                    f"var s=encodeURIComponent('{js_subj}');"
                    f"var b=encodeURIComponent('{js_body}');"
                    f"window.location.href='mailto:{js_to}?subject='+s+'&body='+b;"
                    f'" style="display:inline-block;padding:8px 16px;background:#4CAF50;color:white;border:none;border-radius:4px;cursor:pointer;font-size:0.9em;">📧 {t("send_email_to",name=member["name"])}</button>',
