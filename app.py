...
def tab_team(spreadsheet, members, tasks, user, drive_service):
    ...
    for member in team:
        ...
        with st.expander(f"**{member['name']}** \u2014 {len(mt)} {t('tasks_count')}", expanded=True):
            ...
            if amt:
                email_body_raw = build_email_body(member["name"], amt, app_url)
                email_subj_raw = t("email_subject", date=date.today().strftime("%d.%m.%Y"))
                js_subj = json.dumps(email_subj_raw)
                js_body = json.dumps(email_body_raw)
                js_to = member["email"]
                btn_label = _esc(t("send_email_to", name=member["name"]))
                st.components.v1.html(
                    f'''<button onclick="
                        var s=encodeURIComponent({js_subj});
                        var b=encodeURIComponent({js_body});
                        window.parent.location.href='mailto:{js_to}?subject='+s+'&body='+b;
                    " style="padding:8px 16px;background:#4CAF50;color:white;border:none;border-radius:4px;cursor:pointer;font-size:0.9em;font-family:sans-serif;">📧 {btn_label}</button>''',
                    height=50,
                )
            _archive_team(member, tasks, spreadsheet)
    st.divider()
    all_btn_label = _esc(t("send_all_emails"))
    all_links_js = ""
    for m in team:
        mtl = [tk for tk in vis if tk["assigned_to"] == m["email"] and tk.get("private") != "ja"]
        if mtl:
            body = json.dumps(build_email_body(m["name"], mtl, app_url))
            subj = json.dumps(t("email_subject", date=date.today().strftime("%d.%m.%Y")))
            all_links_js += f"window.open('mailto:{m['email']}?subject='+encodeURIComponent({subj})+'&body='+encodeURIComponent({body}));\n"
    if all_links_js:
        st.components.v1.html(
            f'''<button onclick="{all_links_js}" style="padding:8px 16px;background:#2196F3;color:white;border:none;border-radius:4px;cursor:pointer;font-size:0.9em;font-family:sans-serif;">📧 {all_btn_label}</button>''',
            height=50,
        )
