    ...
    for member in team:
        ...
        with st.expander(f"**{member['name']}** \u2014 {len(mt)} {t('tasks_count')}", expanded=True):
            ...
            if amt:
                ...
                # Email button + copy fallback
                email_body = build_email_body(member["name"], amt, app_url)
                email_subj = t("email_subject", date=date.today().strftime("%d.%m.%Y"))
                ec1, ec2 = st.columns(2)
                ec1.markdown(f'<a href="{ml}" style="display:inline-block;padding:5px 12px;background:#4CAF50;color:white;border-radius:4px;text-decoration:none;font-size:0.85em;">📧 {t("send_email_to",name=member["name"])}</a>', unsafe_allow_html=True)
                if ec2.button(f"📋 {t('copy_email')}", key=f"copy_{member['email']}"):
                    st.code(f"An: {member['email']}\nBetreff: {email_subj}\n\n{email_body}", language=None)
            _archive_team(member, tasks, spreadsheet)
    st.divider()
