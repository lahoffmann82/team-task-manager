...
def ensure_worksheets(spreadsheet):
    ...
    if "projects" not in existing:
        ...
        _migrate_worksheet(spreadsheet, "projects", PROJECT_HEADERS)
...
def build_email_body(name, tl, app_url=""):
    ...
    lines += ["Please update your progress directly via the link above or reply to this email.", "", "Best regards"]
    return "\n".join(lines)
    ...
    for member in team:
        ...
        with st.expander(f"**{member['name']}** \u2014 {len(mt)} {t('tasks_count')}", expanded=True):
            ...
            if amt:
                ...
                btn_text = _esc(t("send_email_to", name=member["name"]))
                st.markdown(
                    f'<a href="{ml}" style="display:inline-block;padding:8px 16px;'
                    f'background:#4CAF50;color:white;border-radius:4px;'
                    f'text-decoration:none;font-size:0.9em;margin:4px 0;">'
                    f'📧 {btn_text}</a>',
...
def tab_projects(spreadsheet, members, projects, user, drive_service):
    ...
    for i, proj in enumerate(projects):
        ...
        # Card
        due = proj.get("due", "")
        due_html = f'<div class="task-info-row"><span class="task-info-label">Fällig</span><span class="task-info-value">{_esc(due)}</span></div>' if due else ""
        edited_by = _esc(proj.get("last_edited_by", ""))
        edited_at = _esc(proj.get("last_edited_at", ""))
        edit_info = f'<div class="task-info-row"><span class="task-info-label">Bearbeitet</span><span class="task-info-value" style="font-weight:400;font-size:0.9em;">{edited_by}, {edited_at}</span></div>' if edited_by else ""

        card_html = (
            '<div class="task-card">'
            '<div class="task-left">'
            f'<div class="task-heading">{_esc(proj["title"])}</div>'
            f'<div class="task-desc-text">{_esc(proj.get("description", ""))}</div>' if proj.get("description") else '<div class="task-left">'
            f'<div class="task-heading">{_esc(proj["title"])}</div>'
        )
        card_html = (
            '<div class="task-card">'
        ...
        if proj.get("description"):
            card_html += f'<div class="task-desc-text">{_esc(proj["description"])}</div>'
        card_html += f'<div class="task-comment-text">{t("project_created_by")}: {_esc(creator)} | {t("project_responsible")}: {", ".join(resp_names) if resp_names else "-"}</div>'
        card_html += '</div><div class="task-right">'
        card_html += f'<div class="task-info-row"><span class="task-info-label">Status</span><span class="badge" style="background:{sc}">{project_status_to_display(sk)}</span></div>'
        card_html += f'{due_html}{edit_info}'
        card_html += '</div></div>'
        ...
        if can_edit and resp_emails:
            ...
            mailto_url = f"mailto:{','.join(resp_emails)}?" + urllib.parse.urlencode({"subject": subj, "body": body}, quote_via=urllib.parse.quote)
            btn_text = _esc(t("project_send"))
            st.markdown(
                f'<a href="{mailto_url}" style="display:inline-block;padding:5px 12px;background:#2196F3;color:white;border-radius:4px;text-decoration:none;font-size:0.85em;margin:4px 0;">📧 {btn_text}</a>',
                unsafe_allow_html=True,
            )
